...
class InventoryItemForm(forms.ModelForm):
    ...
    category = forms.ModelChoiceField(
        queryset=Category.objects.all(), required=False, label="Existing Category"
    )
    ...
    def clean(self):
        ...
        if new_category_name and category:
            self.add_error(
                "new_category",
                "Please provide either an existing category or a new category, not both.",
            )
        ...
        if new_category_name:
            if Category.objects.filter(name=new_category_name).exists():
                self.add_error(
                    "new_category", "Category with this name already exists."
                )
